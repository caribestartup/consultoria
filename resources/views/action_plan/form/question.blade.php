<div class="bd panel-wrapper mT-10 mB-10 question-wrapper"
     @isset($question)
...
>
    <div class="layers">
        <div class="layer w-100 bgc-grey-200 p-15">
            <strong>
                {{ trans_choice('common.question', 1) }} <span class="question-number">{{ $index + 1 }}</span>
        ...
        <div class="layer w-100 p-20 panel-body bgc-white">
            <div class="row mB-20">
                <div class="form-group col-xs-12 col-sm-8 col-md-9 col-lg-9 col-xl-9">
                    ...
                    <input class="form-control question-i" name="action[{{ $action_index }}][question][{{ $index }}][title]" required
                           @isset($question)
                ...
                <div class="form-group col-xs-12 col-sm-4 col-md-3 col-lg-3 col-xl-3">
                    ...
                    <select class="custom-select type-i" name="action[{{ $action_index }}][question][{{ $index }}][type]">
            ...
            <div class="answer-div">
                ...
                <div class="answers">
                    @includeWhen(!isset($question), 'action_plan.form.answer', ['index' => 0, 'question_index' => 0, 'action_index' => $action_index])

                    @isset($question)
                        @foreach($question->options as $option)
                            @include('action_plan.form.answer', ['index' => $loop->index, 'question_index' => $loop->parent->index, 'action_index' => $action_index])
                        @endforeach
                    @endisset
                </div>
            </div>
