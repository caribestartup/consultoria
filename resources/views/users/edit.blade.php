...
@section('content')
	...
	trans_choice('common.user', 2),	__('common.edit')
	]])
				...
				<div class="col-sm-12 col-md-8 col-lg-9">
					...
					<div class="form-group row align-items-center">
						...
						<div class="col-sm-12 col-md-10">
							...
							<input type="hidden" name="boss_id" id="boss-id" @if($user->boss != null)
							value="{{ $user->boss->id }}"
									@endif>
						</div>
					</div>
							...
							<select multiple="multiple" name="department[]" class="department-i multiselect">
								@foreach($departments as $department)
									<option value="{{ $department->id }}"
											@if( array_search($department->id, $department_ids) !== false)
											selected="selected"
											@endif
							...
							<select multiple="multiple" name="group[]" class="group-i multiselect">
								@foreach($groups as $group)
									<option value="{{ $group->id }}"
											@if( array_search($group->id, $group_ids) !== false)
											selected="selected"
											@endif
									>{{ $group->value }}</option>
								@endforeach
					...
					<div class="form-group row align-items-center">
						...
						<div class="col-md-6">
							...
							<label class="switch">
								<input type="checkbox" name="is_coach" id="coach" value="1"
									   @if($user->is_coach)
									   checked
										@endif>
			...
			<div class="row">
				<div class="col-sm-12 col-md-9">
					<div class="form-group row">
						...
						<div class="col-sm-12 col-md-10">
							{{--{!! Form::select("role",[$rolesNames],null,["placeholder" => trans('users.placeholder_role'), "class"=>"form-control custom-select", "optgroup"=>"false"]) !!}--}}
...
@section('js')
	...
	<script>
        $(document).ready(function () {
            ...
            var vEl = document.getElementById('upload-avatar'),
                uploadAvatar = new Croppie(vEl, {
                    viewport: {
                        width: 150,
                        height: 150,
                        type: 'circle'
